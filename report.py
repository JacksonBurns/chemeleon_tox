...
def calculate_multiclass_metrics(predictions_filepath, true_labels_filepath):
    ...
    df_merged = pd.merge(df_predictions, df_true_labels, on='smiles', suffixes=('_pred', '_true'))

    # Check if the merged DataFrame is empty after merging
    if df_merged.empty:
        return {"Error": "No common 'smiles' identifiers found between the two files. "
    ...
    if not is_binary:
        ...
        y_prob_str = df_merged[f'{target}_prob'].apply(lambda x: [float(p) for p in x.split(',')])
    ...
    if not is_binary:
        if y_prob.shape[1] != num_classes:
            print(f"Warning: Number of probability columns ({y_prob.shape[1]}) does not match "
            ...
            try:
                roc_auc_ovr_weighted = roc_auc_score(y_true, y_prob, multi_class='ovr', average='weighted')
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Calculate multiclass classification performance metrics.")
    ...
    parser.add_argument('--true-labels-file', type=str, required=True,
    ...
    if "Error" in metrics:
        ...
        for key, value in metrics.items():
            if key == "Confusion Matrix":
                print(f"\n{key}:\n{np.array(value)}") # Print as numpy array for better readability
            elif key == "Classification Report":
                print(f"\n{key}:\n{value}")
            elif isinstance(value, (float, np.floating)): # Check if it's a float or numpy float
